...
int main()
{
    ...
    printf("* CALCULO DE NOTAS *\n");
        ...
        if (n1 == n2 || n1 == n3){
            printf("A maior nota foi da primeira prova: %1.2f\n", n1);
        }
        if (n2 == n3) {
            printf("A maior nota foi da segunda prova: %1.2f\n", n2);
        }
        if (n1 > n2 && n1 > n3){
            printf("A maior nota foi da primeira prova: %1.2f\n", maiornot);
        }
        ...
        if (n3 > n2 && n3 >n1){
            printf("A maior nota foi da terceira prova: %1.2f\n", maiornot);
        }
        if (n1 < n2 && n1 < n3){
            printf("A menor nota foi da primeira prova: %1.2f\n", menornot);
        }
        if (n2 < n1 && n2 < n3){
            printf("A menor nota foi da segunda prova: %1.2f\n", menornot);
        }
        if (n3 < n2 && n3 <n1){
            printf("A menor nota foi da terceira prova: %1.2f\n", menornot);
        }
        printf("Media: %1.2f\n", maiormed);
    return 0;
}
